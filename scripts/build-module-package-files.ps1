...
$moduleDir = Get-Item "$Destination/$($module.Name)"
...
7z a -tzip -m0=Copy "$($moduleDir.Parent)\$($name).zip" "$($moduleDir.FullName)\*"	

if ($RemoveZippedFolders) {
  Remove-Item -Recurse -Force -Path "$($moduleDir.FullName)"
}
