...
function core.readSmallInteger(address) return ucp.internal.readSmallInteger(address) end
...
function core.readByte(address) return ucp.internal.readByte(address) end
...
function core.readBytes(address, size) return ucp.internal.readBytes(address, size) end
...
function core.readString(address) return ucp.internal.readString(address) end
...
function core.writeInteger(address, value) return ucp.internal.writeInteger(address, value) end
...
function core.writeSmallInteger(address, value) return ucp.internal.writeSmallInteger(address, value) end
...
function core.writeByte(address, value) return ucp.internal.writeByte(address, value) end
...
function core.writeBytes(address, value) return ucp.internal.writeBytes(address, value) end
...
function core.allocate(size) return ucp.internal.allocate(size) end
...
function core.copyMemory(dst, src, length) return ucp.internal.copyMemory(dst, src, length) end
...
function core.AOBScan(target, start, stop)
  ...
  if not result then error("AOB not found: " .. target) end
  return result
end
...
core.Lambda = {
  ...
  __call = function(self, ...) return self.f(...) end
}

---A shorthand function for Lambda:new(f, 1)
function core.ByteLambda(f) return core.Lambda:new(f, 1) end

---A shorthand function for Lambda:new(f, 2)
function core.SmallIntegerLambda(f) return core.Lambda:new(f, 2) end

---A shorthand function for Lambda:new(f, 4)
function core.IntegerLambda(f) return core.Lambda:new(f, 4) end
...
function core.relToLabel(label, offset)
  ...
  return function(address, index, labels)
    if not labels[label] then error("label does not exist: " .. label) end
...
function core.relTo(dst, offset)
  if type(dst) == "string" then return core.relToLabel(dst, offset) end
  local offset = offset or 0
  ...
  for k, v in pairs(code) do
    ...
    elseif type(v) == "function" or getmetatable(v) == core.Lambda then
      ...
      if getmetatable(v) == core.Lambda then 
        ...
        if #vfr ~= v.size then error("Lambda function (size " .. v.size .. ") returned too many values" .. #vfr) end
      else
        vfr = v(address, address-baseAddress, labels)
        if #vfr ~= 4 then error("lambda function: unexpected number of values returned: " .. #vfr) end
      end
      if not vfr then error("Invalid result from lambda function: " .. vfr) end
    ...
    elseif type(v) == "string" then
      if labels[v] ~= address then error("runtime error, label'" .. v .. "' (".. labels[v] .. ") not found at the expected address: " .. address) end
    end
  end
  
  if #result ~= precompiled then error("final size of code (" .. #result .. ") is not equal to the precompilation (" .. precompiled .. ")") end
