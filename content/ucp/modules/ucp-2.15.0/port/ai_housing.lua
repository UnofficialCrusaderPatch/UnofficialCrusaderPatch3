...
return {
    ...
    enable = function(self, config)
        ...
        local code = {
            0xE9, function(address, index, labels)
                local hook = { -- the first 5 bytes are an if condition that just checks if the first house has been built yet
                    0x81, 0xE9, itob(self.value),  -- the value of the slider header gets put into the location of new BinInt32Value()
        ...
        local code = {
            0xB9, itob(self.value),  -- replace the 5 with the user input value from the slider header
        }
